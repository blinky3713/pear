...
annotate :: Lexer Token -> Lexer (Positioned Token)
annotate p = do
  spos <- P.getPosition
  a <- p
  epos <- P.getPosition
  pure $ Positioned (SourceSpan spos epos) a
...
pearLexer :: String -> Either P.ParseError [Positioned Token]
pearLexer = P.parse (PC.many1 ps) ""
  where
    ps :: Lexer (Positioned Token)
...
type TokenParser a = P.ParsecT [Positioned Token] () Identity a

token :: (Token -> Maybe a) -> TokenParser a
token f = P.token (prettyPrintToken . positionedData) (ssStart . positionedSource) (f . positionedData)
